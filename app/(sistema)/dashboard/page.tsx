...
import { TrendingUp, DollarSign, Target, Gift, Building2, Calendar, ArrowUpRight, ArrowDownRight } from 'lucide-react'
import { arbitragensAPI, freebetsAPI, casasAPI, freespinsAPI, Arbitragem, Freebet, Casa, FreeSpin } from '@/lib/api'
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [arbitragensData, freebetsData, casasData, freespinsData] = await Promise.all([
  ...
  const lucroExtraidoFreebet = freebets.reduce((sum, f) => sum + (f.valorExtraido || 0), 0)
  const lucroArbitragens = arbitragens.filter(a => a.status === 'executada').reduce((sum, a) => sum + a.lucroEsperado, 0)
  const lucroTotal = lucroArbitragens + lucroRodadasGratis + lucroExtraidoFreebet
