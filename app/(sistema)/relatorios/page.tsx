...
function calcularLucroMensal(arbitragens, freebets, periodo) {
  ...
  // Ordenar por data
  const arr = Object.values(meses).sort((a, b) => new Date('01 ' + a.mes) - new Date('01 ' + b.mes))
...
function calcularPorCasa(arbitragens, freebets) {
  const casas = {}
  arbitragens.forEach(a => {
    if (a.casa1) {
...
function calcularPorEsporte(arbitragens) {
  const esportes = {}
  arbitragens.forEach(a => {
...
function calcularPorEstrategia(arbitragens, freebets) {
...
export default function RelatoriosPage() {
  ...
  const [arbitragens, setArbitragens] = useState([])
